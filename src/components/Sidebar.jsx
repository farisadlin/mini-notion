    ...
    <aside className="border-b border-slate-800 bg-slate-950/95 lg:fixed lg:inset-y-0 lg:left-0 lg:z-40 lg:w-72 lg:border-b-0 lg:border-r">
      <div className="flex h-full flex-col gap-5 px-4 py-5">
        ...
        <div className="space-y-2">
          <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">{t('quickAdd')}</p>
          <div className="grid grid-cols-3 gap-2 lg:grid-cols-1">
            <button className="rounded-md bg-sky-500 px-3 py-2 text-sm font-semibold text-slate-950 hover:bg-sky-400" type="button" onClick={() => onQuickAdd('tasks')}>{t('quickAdd.task')}</button>
            <button className="rounded-md bg-emerald-400 px-3 py-2 text-sm font-semibold text-slate-950 hover:bg-emerald-300" type="button" onClick={() => onQuickAdd('courses')}>{t('quickAdd.course')}</button>
            <button className="rounded-md bg-violet-400 px-3 py-2 text-sm font-semibold text-slate-950 hover:bg-violet-300" type="button" onClick={() => onQuickAdd('notes')}>{t('quickAdd.note')}</button>
          </div>
        </div>

        <div className="space-y-2">
          <div className="flex items-center justify-between gap-2">
            <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">{t('sidebar.courses')}</p>
            {selectedCourseId && (
              <button className="text-xs text-slate-400 hover:text-slate-100" type="button" onClick={onClearCourse}>{t('sidebar.all')}</button>
            )}
          </div>
        ...
        <div className="border-t border-slate-800 pt-4">
          ...
          >
            <span>{t('language')}: {i18n.language === 'id' ? 'Indonesia' : 'English'}</span>
          </button>
        </div>
      </div>
    </aside>
