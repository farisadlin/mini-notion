...
const labelKeys = {
  tasks: "entity.tasks",
  courses: "entity.coursesShort",
  notes: "entity.notes",
};
const addLabelKeys = {
  tasks: "table.addTasks",
  courses: "table.addCourses",
  notes: "table.addNotes",
};
...
function TableView({
  ...
  const rows = useMemo(() => {
    ...
    return [...source]
      ...
      .sort((a, b) => {
        const av = String(
          Array.isArray(a[sort.key])
            ? a[sort.key].join(", ")
            : (a[sort.key] ?? ""),
        );
        const bv = String(
          Array.isArray(b[sort.key])
            ? b[sort.key].join(", ")
            : (b[sort.key] ?? ""),
        );
    ...
    const value =
      editing.key === "tags"
        ? String(draft)
            ...
            .filter(Boolean)
        : draft;
    onUpdate(entity, editing.id, {
      [editing.key]: type === "number" ? Number(value) : value,
    });
    setEditing(null);
  };
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="flex rounded-lg border border-slate-800 bg-slate-900 p-1">
          {Object.entries(labelKeys).map(([id, labelKey]) => (
            <button
              className={`rounded-md px-3 py-2 text-sm ${entity === id ? "bg-slate-700 text-slate-50" : "text-slate-400 hover:text-slate-100"}`}
              key={id}
              type="button"
        ...
        <input
          className="min-w-56 flex-1 rounded-md border border-slate-800 bg-slate-900 px-3 py-2 text-sm text-slate-100 outline-none focus:border-sky-400"
          value={filter}
                    ...
                    <td className="max-w-72 px-4 py-3 align-top" key={key}>
                      {editing?.id === item.id && editing?.key === key ? (
                        ...
                        <button
                          ...
                          onClick={() =>
                            type !== "readonly" && startEdit(item, key)
                          }
                        >
                          ...
                          <span className="truncate">
                            {displayValue(entity, key, item, courses, t)}
                          </span>
                        </button>
                      )}
                    </td>
