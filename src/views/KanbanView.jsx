...
function TaskCard({ task, course, onEdit }) {
  ...
  return (
    <article
      ...
      {...listeners}
    >
      <button className="w-full text-left" type="button" onClick={() => onEdit('tasks', task)}>
        <div className="flex items-start justify-between gap-3">
          <p className="font-medium text-slate-100">{task.title}</p>
          {course && <span className="mt-1 h-2.5 w-2.5 shrink-0 rounded-full" style={{ backgroundColor: course.color }} />}
        </div>
        {task.deadline && <p className="mt-2 text-xs text-slate-400">{new Date(task.deadline).toLocaleString([], { dateStyle: 'medium', timeStyle: 'short' })}</p>}
      </button>
    </article>
  )
}
