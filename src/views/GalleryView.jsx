  ...
  return (
    <section className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-2xl font-semibold text-slate-50">{t('gallery.title')}</h2>
          <p className="text-sm text-slate-400">{t('gallery.subtitle')}</p>
        </div>
        <button className="rounded-md bg-violet-400 px-4 py-2 text-sm font-semibold text-slate-950 hover:bg-violet-300" type="button" onClick={() => onAdd('notes')}>{t('gallery.addNote')}</button>
      </div>
        ...
        {notes.map((note) => (
          <article className="rounded-lg border border-slate-800 bg-slate-900/70 p-4 transition hover:border-slate-600" key={note.id}>
            ...
            <div className="mt-4 flex gap-2">
              <button className="rounded-md border border-slate-700 px-2 py-1 text-xs text-slate-200 hover:bg-slate-800" type="button" onClick={() => onEdit('notes', note)}>{t('button.edit')}</button>
              <button className="rounded-md border border-red-500/50 px-2 py-1 text-xs text-red-200 hover:bg-red-500/10" type="button" onClick={() => onDelete('notes', note.id)}>{t('button.delete')}</button>
            </div>
          </article>
        ))}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 px-4 backdrop-blur-sm">
          <article className="w-full max-w-2xl rounded-lg border border-slate-700 bg-slate-900 p-5">
            <div className="flex items-start justify-between gap-4">
              ...
              <button className="rounded-md px-2 py-1 text-slate-400 hover:bg-slate-800 hover:text-slate-100" type="button" onClick={() => setActiveNote(null)}>{t('button.close')}</button>
            </div>
