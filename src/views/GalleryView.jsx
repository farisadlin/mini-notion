  ...
  return (
    <section className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <button
          className="rounded-md bg-violet-400 px-4 py-2 text-sm font-semibold text-slate-950 hover:bg-violet-300"
          type="button"
          onClick={() => onAdd("notes")}
        >
          {t("gallery.addNote")}
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        {notes.map((note) => (
          <article
            className="rounded-lg border border-slate-800 bg-slate-900/70 p-4 transition hover:border-slate-600"
            key={note.id}
          >
            <button
            ...
            <div className="mt-4 flex gap-2">
              ...
              <button
                ...
                type="button"
                onClick={() => onDelete("notes", note.id)}
              >
                {t("button.delete")}
              </button>
            </div>
